...
export function registerHandlers(bot: Bot): void {
  ...
  bot.command("mks_test", async (ctx) => {
    const status = await ctx.reply("📝 Генерирую МКС АОСР+РЭР (Салтыковка тест)...");
    try {
      ...
      logger.error({ err }, "Ошибка /mks_test");
      await ctx.api.editMessageText(ctx.chat.id, status.message_id, `❌ Ошибка: ${err instanceof Error ? err.message : "неизвестная ошибка"}`);
    }
  });
